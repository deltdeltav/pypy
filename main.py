...
# === НОВАЯ СИСТЕМА УРОВНЕЙ И БИОМОВ ===
LEVEL = 1              
WAVE_LIMIT = 50       
BASE_START_MONEY = 500 

# Биомы меняются каждые 10 уровней (500 волн)
BIOMES = [
...
def reset():
    ...
    wave = 1
    lives = 20
    money = BASE_START_MONEY * LEVEL + 200 * (LEVEL - 1)
...
while running:
        ...
        if ev.type == pygame.KEYDOWN:
            ...
            if not menu_open:
                ...
                if ev.key == pygame.K_LEFT:
                    if unit_scroll > 0: unit_scroll -= 1; pygame.time.wait(100)
                if ev.key == pygame.K_RIGHT:
                    if unit_scroll + V_MAX_VISIBLE < len(all_keys_temp): unit_scroll += 1; pygame.time.wait(100)
            ...
            if ev.button == 1:
                ...
                if menu_open:
                    ...
                    for t in range(3):
                        ...
                        if rect.collidepoint(mx, my): menu_tab = t; clicked_menu = True
                    
                    close_rect_tmp = pygame.Rect(menu_x_tmp + 20, menu_y_tmp + menu_h_tmp - 80, 90, 60)
                    if close_rect_tmp.collidepoint(mx, my): menu_open = False; clicked_menu = True
                    ...
                    if not clicked_menu:
                        ...
                        if menu_tab == 0: # UPGRADES
                            # Формула прокачки: 1=5, 2=15, 3=30... (L*(L+1)/2)*5
                            max_allowed_upgrades = (LEVEL * (LEVEL + 1) // 2) * 5
                            ...
                            if pygame.Rect(left_x, start_y_content, scroll_w, item_h_tmp).collidepoint(mx, my):
                                if upgrade_scroll > 0: upgrade_scroll -= 1; pygame.time.wait(150)
                            elif pygame.Rect(right_x, start_y_content, scroll_w, item_h_tmp).collidepoint(mx, my):
                                if upgrade_scroll < len(all_upgrades) - 5: upgrade_scroll += 1; pygame.time.wait(150)
                            else:
                                ...
                                for i, (name, key, lvl, costs, col) in enumerate(visible_ups):
                                    ...
                                    if rect_card.collidepoint(mx, my):
                                        ...
                                        # Проверка: уровень меньше лимита И есть деньги И есть цена в списке
                                        if lvl < max_allowed_upgrades and lvl < len(costs) and money >= cost:
                                            money -= cost; global_upgrades[key] += 1
                                            if key == 'energy_cap':
                            ...
                            if pygame.Rect(left_x, start_y_content, scroll_w, item_h_tmp).collidepoint(mx, my):
                                if shop_scroll > 0: shop_scroll -= 1; pygame.time.wait(150)
                            elif pygame.Rect(right_x, start_y_content, scroll_w, item_h_tmp).collidepoint(mx, my):
                                if shop_scroll < len(SHOP_UNITS) - 5: shop_scroll += 1; pygame.time.wait(150)
                            else:
                                card_x_start = menu_x_tmp + 100
                                card_width = menu_w_tmp - 200
                                for i, k in enumerate(visible_keys):
                                    ...
                                    if rect_card.collidepoint(mx, my):
                                        if crystals >= v['cost_crystals'] and k not in UNITS:
                                            ...
                                            save_game(crystals, list(UNITS.keys())); pygame.time.wait(200)
                    ...
                    if len(all_unit_keys) > V_MAX_VISIBLE and left_arrow_rect.collidepoint(mx, my):
                        if unit_scroll > 0: unit_scroll -= 1; clicked_ui = True; pygame.time.wait(100)
                    elif len(all_unit_keys) > V_MAX_VISIBLE and right_arrow_rect.collidepoint(mx, my):
                        if unit_scroll + V_MAX_VISIBLE < len(all_unit_keys): unit_scroll += 1; clicked_ui = True; pygame.time.wait(100)
                    else:
                        for i, k in enumerate(visible_keys_game):
                            x = V_MENU_X; y = V_MENU_Y_START + i * (V_BTN_H + V_GAP)
                            if pygame.Rect(x, y, V_BTN_W, V_BTN_H).collidepoint(mx, my):
                                selected_unit = k; clicked_ui = True; break
                ...
                if not menu_open:
                    ...
                    if 0 <= c < COLS and 0 <= r < ROWS:
                        for t in towers:
                            if t.c == c and t.r == r:
                                money += int(t.data['cost'] * 0.2)
                                grid_map[r][c] = 0
                                towers.remove(t)
                                fx.spawn_explosion(t.screen_x, t.screen_y, (0, 255, 0), count=10)
                                break
            
        if ev.type == pygame.MOUSEBUTTONUP and ev.button==1: 
            if dragging:
                dx = abs(pygame.mouse.get_pos()[0] - drag_start_pos[0])
                dy = abs(pygame.mouse.get_pos()[1] - drag_start_pos[1])
                if dx < 5 and dy < 5 and not menu_open:
    ...
    # === LOGIC ===
    if energy < max_energy: energy += ENERGY_REGEN
    if energy > max_energy: energy = max_energy
    if dark_matter < max_dark_matter: dark_matter += DARK_MATTER_REGEN
    if dark_matter > max_dark_matter: dark_matter = max_dark_matter

    level_difficulty = 1 + (LEVEL - 1) * 2
    mult = level_difficulty * (1 + wave * 0.02) + (global_upgrades['income'] * 0.05)
    ...
    if enemies_to_spawn==0 and not enemies and wave_active:
        ...
        if wave >= WAVE_LIMIT:
            ...
            wave = 1
            money += 500 * LEVEL
            fx.spawn_explosion(W//2, H//2, (255, 215, 0), count=50)
            fx.add_shake(15)
        else:
            wave += 1
        start_wave()
    ...
    for t in towers:
        ...
        if t.data['type'] == 'strike' and old_cd > 0 and t.cd == 0 and energy >= 50: energy -= 50
        if t.data['type'] == 'chain' and old_cd > 0 and t.cd == 0 and energy >= 20: energy -= 20
        if t.data['type'] == 'pull':
            has_target = any(math.hypot(e.screen_x - t.screen_x, e.screen_y - t.screen_y) < t.data['range'] * TILE_W for e in enemies)
            if has_target and dark_matter >= 0.5: dark_matter -= 0.5
    ...
    font_top = pygame.font.SysFont('consolas', 22, bold=True)
    if wave % 500 == 0 and not wave_active:
        top_txt = " CHECKPOINT! LEVEL COMPLETE "
        top_col = (255, 215, 0)
    else:
        top_txt = f"LVL {LEVEL} | {current_biome['name']} | WAVE {wave}/{WAVE_LIMIT}"
        top_col = (220, 220, 220)
    txt_surf = font_top.render(top_txt, True, top_col)
    screen.blit(txt_surf, (W//2 - txt_surf.get_width()//2, 20))
    ...
    # Добавил CR обратно в строку статистики
    info = f"LVL {LEVEL} | W: {wave}/{WAVE_LIMIT} | $ {money} | CR: {crystals} | T: {len(towers)}/{current_limit} | ⚡ {int(energy)} | 🌑 {int(dark_matter)} | HP: {lives}"
    screen.blit(font.render(info, True, Colors.ACCENT), (10, 10))
    
    # Полоски ресурсов (Энергия, Материя, HP)
    ...
    pygame.draw.rect(screen, (255, 50, 50), (10, 70, 100 * (lives / max_lives_display), 8), border_radius=2)
    ...
    if len(all_unit_keys) > V_MAX_VISIBLE:
        ...
        pygame.draw.rect(screen, (60,60,80), (V_MENU_X, arrow_y_pos, 70, 30), border_radius=4)
        screen.blit(font_arrow.render("< PREV", True, Colors.WHITE), (V_MENU_X + 5, arrow_y_pos + 5))
        pygame.draw.rect(screen, (60,60,80), (V_MENU_X + 80, arrow_y_pos, 70, 30), border_radius=4)
        screen.blit(font_arrow.render("NEXT >", True, Colors.WHITE), (V_MENU_X + 85, arrow_y_pos + 5))
